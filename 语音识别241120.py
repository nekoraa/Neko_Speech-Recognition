...
def 录音并识别(输入设备):
    ...
    while True:
        ...
        if len(音频缓冲区) > math.floor(阈值长度/时长):
            音频缓冲区 = 音频缓冲区[- math.floor(阈值长度/时长):]
        else:
            pass
        ...
        if 是否有声音活动(音频):
            ...
            if 计数次数 % 实时翻译速率 == 0:
                threading.Thread(target=识别模型, args=(np.concatenate(音频缓冲区),)).start()
            ...
            if 静音计数 >= 静音阈值:
                ...
                # print(type(全部音频))
                # print(全部音频.shape)
                threading.Thread(target=识别模型翻译, args=(np.concatenate(音频缓冲区),)).start()
                break
        else:
            pass
